  ...
  if (!session) {
    return (
      <PageContainer>
        ...
        <form className="mt-6 max-w-sm" onSubmit={handleLogin}>
          ...
          <input
            ...
            className="mt-2 w-full rounded-xl border border-border bg-card p-3 text-sm text-foreground focus:border-accent focus:outline-none"
          />
          ...
          {loginError && <p className="mt-3 text-sm font-medium text-accent">{loginError}</p>}
    ...
    <PageContainer>
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex min-h-11 items-center rounded-full border border-border bg-card px-4 text-sm font-medium text-muted"
        >
          로그아웃
        </button>
      </div>
      ...
      <details className="group mt-6 rounded-xl border border-border bg-card">
        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-medium text-foreground marker:content-none">
          승인 기준
          <span
            aria-hidden
        ...
        <div className="border-t border-border px-5 py-4 text-sm leading-relaxed text-muted">
          <p className="font-medium text-foreground">다음은 승인하지 않습니다.</p>
      ...
      <div className="mt-6 space-y-3">
        {loadingReviews && <p className="text-sm text-muted">불러오는 중…</p>}

        {!loadingReviews && listError && (
          <p className="text-sm font-medium text-accent">{listError}</p>
        )}
              ...
              <div className="mt-4 flex gap-2">
                {activeTab === "pending" && (
                  <>
                    <button
                      ...
                      className="flex min-h-11 flex-1 items-center justify-center rounded-xl bg-accent px-4 text-sm font-medium text-white transition-colors hover:bg-accent-2 disabled:opacity-40"
                    >
                      승인
                    </button>
                    <button
                      ...
                      className="flex min-h-11 flex-1 items-center justify-center rounded-xl border border-border bg-card px-4 text-sm font-medium text-muted transition-colors disabled:opacity-40"
                    >
                      거절
                    </button>
                  </>
                ...
                {activeTab === "approved" && (
                  <button
                    ...
                    className="flex min-h-11 w-full items-center justify-center rounded-xl border border-border bg-card px-4 text-sm font-medium text-muted transition-colors disabled:opacity-40"
                  >
                    승인 취소
                  </button>
                )}
